...
void applyUADefaults(RenderNode* node) {
    ...
    if (tag == "img") {
        ...
        if (wIt != node->attrs.end() && !wIt->second.empty()) {
            try {
                int wv = std::stoi(wIt->second);
                if (wv > 0) {
                    s.width = Length{(float)wv, Length::Px};
                    s.hasWidth = true;
                }
            } catch (...) {}
        }
        auto hIt = node->attrs.find("height");
        if (hIt != node->attrs.end() && !hIt->second.empty()) {
            try {
                int hv = std::stoi(hIt->second);
                if (hv > 0) {
                    s.height = Length{(float)hv, Length::Px};
                    s.hasHeight = true;
                }
            } catch (...) {}
        }
